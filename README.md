...
A Docker-like container runtime built from scratch on raw Linux kernel
...
1. **Pull** the image from Docker Hub, cache its layers — `src/registry/`
2. **Mount** those layers with OverlayFS (shared read-only base + private writable layer) — `src/fs/overlay.cpp`
...
Steps 1-2 and 5-6 run in the parent process, around a single `clone()`
call; the child waits on a small pipe until that setup finishes before it
execs anything.

## Requirements

- A real Linux host (not macOS/WSL) — this calls Linux syscalls
...
src/cgroups/         cgroup v2 CPU / memory / pids limits
...
- **Filesystem** — the image's layers are combined into one OverlayFS
  mount (read-only base + a fresh writable layer), and `pivot_root` swaps
  that in as the container's actual root — not just a `chroot`, so there's
...
- **Networking** — each container gets a veth pair wired to a shared
  ...
  the internet.
- **Registry client** — a small Docker Hub v2 client (`libcurl` +
  `nlohmann/json`) that resolves multi-arch manifests, downloads layers,
  and caches them locally, so any public image can be pulled and run.
...
- **PID/hostname isolation** — container's shell is PID 1 in its own
  namespace; host's hostname is untouched.
...
- **Memory limit** — a memory hog was OOM-killed by the kernel at exactly
  the configured limit (`dmesg`: `mem_cgroup_out_of_memory`); host
  unaffected.
...
- **Networking** — outbound internet access works via NAT; two
  ...
  bridge in ~0.06ms.
- **Real images** — `alpine:latest` and `python:3.11-slim` (different
  base OS, multiple layers) both pulled live from Docker Hub and run
  end-to-end.
- **Repeatability** — `scripts/test-harness.sh`: 100% pass rate over 26
  runs, zero leaked mounts/cgroups/veth interfaces, p50 startup 0.736s /
  p95 0.920s.

## Known limitations

The most important ones — full list and why in [DESIGN.md](DESIGN.md):
...
- No image config parsing (`CMD`/`ENTRYPOINT`/`ENV`) — every run needs an
  explicit command.
- No layer digest verification, no private registries.
- Cleanup relies on normal process exit — a `SIGKILL` to the runtime
  itself can leave a stray cgroup or mount behind.
- One container per invocation — no daemon, no orchestration.
